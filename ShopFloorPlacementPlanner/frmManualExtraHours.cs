...
namespace ShopFloorPlacementPlanner
{
    public partial class frmManualExtraHours : Form
    {
        ...
        private void cmbDept_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string sql = "SELECT staff_id FROM dbo.view_planner_punch_staff WHERE date_plan = '" + _selected_date.ToString("yyyyMMdd") + "' AND department = '" + cmbDept.Text + "' AND  [Staff Name] = '" + _staff_name + "'";
            ...
            using (SqlConnection conn = new SqlConnection(connectionStrings.ConnectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                {
                    string getdata = (string)cmd.ExecuteScalar().ToString();
